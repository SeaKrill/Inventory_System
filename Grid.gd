...
func insert_item(item):
	...
	if is_grid_space_available(g_pos.x, g_pos.y, item_size.x, item_size.y):
		...
		items.append(item)
		GameState.data.player.inventory.append([item.get_meta("id"), item.rect_global_position.x, item.rect_global_position.y, item.rotated])
		return true
	else:
		return false
...
func grab_item(pos):
	...
	items.remove(items.find(item))
	GameState.data.player.inventory.remove(GameState.data.player.inventory.find([item.get_meta("id"), item.rect_global_position.x, item.rect_global_position.y, item.rotated]))
	return item
